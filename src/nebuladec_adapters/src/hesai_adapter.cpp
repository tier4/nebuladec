...
// NEBULADEC_ACCELERATED_HESAI opt-out: defaults to enabled. Set to "0" to force
// the upstream nebula::drivers::HesaiDriver; useful for A/B timing
// comparisons or for falling back if AcceleratedHesaiDriver regresses on a new
// sensor variant. Read once at construction so the choice is stable for
// the lifetime of the adapter.
bool accelerated_hesai_opted_out() noexcept
{
  const char * raw = std::getenv("NEBULADEC_ACCELERATED_HESAI");
...
HesaiAdapter::HesaiAdapter(const Identity & identity) : identity_(identity)
{
  ...
  try {
    ...
    if (AcceleratedHesaiDriver::supports(identity.model) && !accelerated_hesai_opted_out()) {
      accelerated_driver_ =
        std::make_unique<AcceleratedHesaiDriver>(config, calibration, logger, callback);
    } else {
  ...
  const std::vector<std::uint8_t> & packet, double /*stamp_sec*/)
{
  ...
  if (accelerated_driver_) {
    NEBULADEC_PROFILE_SCOPE("accelerated_hesai_driver_parse_cloud_packet");
    accelerated_driver_->parse_cloud_packet(packet);
  } else {
...
std::optional<nebula::drivers::NebulaPointCloudPtr> HesaiAdapter::flush()
{
  ...
  for (const auto & pkt : first_scan_packets_) {
    if (accelerated_driver_) {
      accelerated_driver_->parse_cloud_packet(pkt);
    } else {
